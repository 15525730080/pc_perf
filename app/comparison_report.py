...
# 新增高级分析工作表的常量定义
SIGNIFICANT_THRESHOLD = 0.05  # p值显著性阈值
HIGH_CONFIDENCE = 95  # 高置信度阈值
...
async def create_comparison_excel(tasks_data: Dict[str, Any], report_name: Optional[str] = None, 
    ...
    # 如果有高级分析数据，创建高级分析工作表
    has_advanced_analysis = "advanced_analysis" in tasks_data
    if has_advanced_analysis:
        ws_advanced = wb.create_sheet(title="高级分析")
    ...
    # 填充高级分析工作表
    if has_advanced_analysis:
        populate_advanced_analysis_sheet(ws_advanced, tasks_data["advanced_analysis"])
    
    # 设置列宽
    sheet_list = [ws_summary, ws_details, ws_charts]
    if has_advanced_analysis:
        sheet_list.append(ws_advanced)
...
def populate_advanced_analysis_sheet(ws, advanced_analysis: Dict[str, Any]) -> None:
    """填充高级分析工作表"""
    # 添加标题
    ...
    # 添加统计显著性分析
    ws.cell(row=row, column=1, value="一、统计显著性分析").font = HEADER_FONT
    row += 1
    
    # 添加说明
    ws.cell(row=row, column=1, value="统计显著性分析用于检验两个版本间的性能差异是否具有统计学意义。")
    row += 1
    ws.cell(row=row, column=1, value="p值小于0.05表示差异具有统计显著性；置信度越高表示差异越可靠。")
    row += 2
    
    # 添加表头
    headers = ["指标", "基准版本均值", "对比版本均值", "差异", "变化率(%)", "p值", "置信度(%)", "差异显著性"]
    for col, header in enumerate(headers, 1):
        cell = ws.cell(row=row, column=col, value=header)
        cell.font = SUBHEADER_FONT
        cell.alignment = CENTER_ALIGNMENT
    ...
    # 添加统计显著性分析数据
    stat_data = advanced_analysis.get("statistical_significance", {})
    for metric, data in stat_data.items():
        col = 1
        # 指标名称
        ws.cell(row=row, column=col, value=metric_name_map.get(metric, metric))
        col += 1
        
        # 基准版本均值
        ws.cell(row=row, column=col, value=data.get("base_mean")).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 对比版本均值
        ws.cell(row=row, column=col, value=data.get("comp_mean")).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 差异
        diff = data.get("diff")
        ws.cell(row=row, column=col, value=diff).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 变化率(%)
        percent = data.get("percent_change")
        ws.cell(row=row, column=col, value=percent).alignment = CENTER_ALIGNMENT
        col += 1
        
        # p值
        p_value = data.get("p_value")
        cell = ws.cell(row=row, column=col, value=p_value)
        cell.alignment = CENTER_ALIGNMENT
        # p值小于显著性水平时突出显示
        ...
        cell = ws.cell(row=row, column=col, value=confidence)
        cell.alignment = CENTER_ALIGNMENT
        # 高置信度突出显示
        ...
        cell = ws.cell(row=row, column=col, value="显著" if is_significant else "不显著")
        cell.alignment = CENTER_ALIGNMENT
        if is_significant:
            cell.fill = PatternFill(start_color="FFEB9C", end_color="FFEB9C", fill_type="solid")
    ...
    # 添加异常值检测结果
    ws.cell(row=row, column=1, value="二、异常值检测").font = HEADER_FONT
    row += 1
    
    # 添加说明
    ws.cell(row=row, column=1, value="异常值检测用于发现明显偏离正常范围的性能数据点。")
    row += 1
    ws.cell(row=row, column=1, value="异常值增多可能表示性能不稳定或出现了性能问题。")
    row += 2
    
    # 添加表头
    headers = ["指标", "基准版本异常值比例(%)", "对比版本异常值比例(%)", "异常值变化", "异常值上下限(基准)", "异常值上下限(对比)"]
    for col, header in enumerate(headers, 1):
        cell = ws.cell(row=row, column=col, value=header)
        cell.font = SUBHEADER_FONT
        cell.alignment = CENTER_ALIGNMENT
    ...
    # 添加异常值检测数据
    anomaly_data = advanced_analysis.get("anomaly_detection", {})
    for metric, data in anomaly_data.items():
        ...
        # 基准版本异常值比例
        base_percent = data.get("base", {}).get("anomalies_percent", 0)
        ws.cell(row=row, column=col, value=base_percent).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 对比版本异常值比例
        comp_percent = data.get("compare", {}).get("anomalies_percent", 0)
        cell = ws.cell(row=row, column=col, value=comp_percent)
        cell.alignment = CENTER_ALIGNMENT
        col += 1
        
        # 异常值变化
        change = comp_percent - base_percent
        cell = ws.cell(row=row, column=col, value=f"{'+' if change > 0 else ''}{change:.2f}%")
        cell.alignment = CENTER_ALIGNMENT
        # 变化明显时突出显示
        if change > 5:  # 异常值增加超过5%
            cell.fill = WORSE_FILL
        elif change < -5:  # 异常值减少超过5%
            cell.fill = BETTER_FILL
        col += 1
        
        # 异常值上下限(基准)
        base_bounds = data.get("base", {}).get("bounds", {})
        base_bounds_str = f"下: {base_bounds.get('lower', 0):.2f}, 上: {base_bounds.get('upper', 0):.2f}"
        ws.cell(row=row, column=col, value=base_bounds_str).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 异常值上下限(对比)
        comp_bounds = data.get("compare", {}).get("bounds", {})
        comp_bounds_str = f"下: {comp_bounds.get('lower', 0):.2f}, 上: {comp_bounds.get('upper', 0):.2f}"
        ws.cell(row=row, column=col, value=comp_bounds_str).alignment = CENTER_ALIGNMENT
    ...
    # 添加性能稳定性分析
    ws.cell(row=row, column=1, value="三、性能稳定性分析").font = HEADER_FONT
    row += 1
    
    # 添加说明
    ws.cell(row=row, column=1, value="性能稳定性分析用于评估性能指标的波动程度，稳定性评分越高表示性能越稳定。")
    row += 2
    
    # 添加表头
    headers = ["指标", "基准版本稳定性评分", "对比版本稳定性评分", "稳定性变化", "稳定性趋势"]
    for col, header in enumerate(headers, 1):
        cell = ws.cell(row=row, column=col, value=header)
        cell.font = SUBHEADER_FONT
        cell.alignment = CENTER_ALIGNMENT
    ...
    # 添加性能稳定性数据
    stability_data = advanced_analysis.get("performance_stability", {})
    for metric, data in stability_data.items():
        col = 1
        # 指标名称
        ws.cell(row=row, column=col, value=metric_name_map.get(metric, metric))
        col += 1
        ...
        # 基准版本稳定性评分
        base_score = data.get("base", {}).get("stability_score", 0)
        ws.cell(row=row, column=col, value=base_score).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 对比版本稳定性评分
        comp_score = data.get("compare", {}).get("stability_score", 0)
        ws.cell(row=row, column=col, value=comp_score).alignment = CENTER_ALIGNMENT
        col += 1
        
        # 稳定性变化
        change = comp_score - base_score
        cell = ws.cell(row=row, column=col, value=f"{'+' if change > 0 else ''}{change:.2f}")
        cell.alignment = CENTER_ALIGNMENT
        col += 1
        ...
        cell = ws.cell(row=row, column=col, value="改善" if improved else "下降")
        cell.alignment = CENTER_ALIGNMENT
        cell.fill = BETTER_FILL if improved else WORSE_FILL
    ...
    # 添加性能瓶颈分析
    ws.cell(row=row, column=1, value="四、性能瓶颈分析").font = HEADER_FONT
    row += 1
    
    bottleneck_data = advanced_analysis.get("bottleneck_analysis", {})
    has_bottlenecks = bottleneck_data.get("has_bottlenecks", False)
    
    # 添加说明
    if has_bottlenecks:
        ws.cell(row=row, column=1, value=f"检测到 {bottleneck_data.get('bottlenecks_count', 0)} 个潜在的性能瓶颈。")
    else:
        ws.cell(row=row, column=1, value="未检测到显著的性能瓶颈。")
    row += 2
    
    # 如果有瓶颈，添加瓶颈详情
    if has_bottlenecks:
        # 添加表头
        headers = ["瓶颈指标", "变化率(%)", "置信度(%)", "瓶颈原因"]
        for col, header in enumerate(headers, 1):
            cell = ws.cell(row=row, column=col, value=header)
            cell.font = SUBHEADER_FONT
            cell.alignment = CENTER_ALIGNMENT
        ...
        # 添加瓶颈数据
        for bottleneck in bottleneck_data.get("potential_bottlenecks", []):
            col = 1
            # 瓶颈指标
            ...
            # 变化率(%)
            percent = bottleneck.get("percent_change", 0)
            ws.cell(row=row, column=col, value=percent).alignment = CENTER_ALIGNMENT
            col += 1
            
            # 置信度(%)
            confidence = bottleneck.get("significance", {}).get("confidence", 0)
            ws.cell(row=row, column=col, value=confidence).alignment = CENTER_ALIGNMENT
            col += 1
            
            # 瓶颈原因
            reasons = bottleneck.get("reasons", [])
            ws.cell(row=row, column=col, value="\n".join(reasons))
            ws.row_dimensions[row].height = 15 * len(reasons)  # 调整行高
